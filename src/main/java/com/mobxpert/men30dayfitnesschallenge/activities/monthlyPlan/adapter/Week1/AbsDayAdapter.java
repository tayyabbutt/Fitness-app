...
public class AbsDayAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AbsDayAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
     ...
     /*   if (myWorkoutList.get(position).isExerciseDone() == true) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek1Day4Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
