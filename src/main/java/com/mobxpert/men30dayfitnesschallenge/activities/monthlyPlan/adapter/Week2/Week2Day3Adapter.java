package com.mobxpert.men30dayfitnesschallenge.activities.monthlyPlan.adapter.Week2;
...
public class Week2Day3Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Week2Day3Adapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
     ...
     /*   if (myWorkoutList.get(position).isExerciseDone() == true) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek2Day3Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
