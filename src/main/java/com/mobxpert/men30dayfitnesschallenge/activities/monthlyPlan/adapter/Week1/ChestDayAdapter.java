...
public class ChestDayAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public ChestDayAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek1Day1Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
        } /*else {
            holder.bicepsName.setText(myWorkoutList.get(position).getName());
        }*/
