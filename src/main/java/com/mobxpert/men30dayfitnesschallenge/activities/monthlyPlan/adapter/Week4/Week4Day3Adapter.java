...
public class Week4Day3Adapter  extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Week4Day3Adapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
     ...
     /*   if (myWorkoutList.get(position).isExerciseDone() == true) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek4Day3Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
