package com.mobxpert.men30dayfitnesschallenge.activities.monthlyPlan.adapter.Week3;
...
public class Week3Day6Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Week3Day6Adapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
     ...
     /*   if (myWorkoutList.get(position).isExerciseDone() == true) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek3Day6Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
