...
public class Week3Day5Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Week3Day5Adapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
     ...
     /*   if (myWorkoutList.get(position).isExerciseDone() == true) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek2Day5Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
