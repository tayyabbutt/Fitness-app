...
public class Week3Day2Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Week3Day2Adapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
     ...
     /*   if (myWorkoutList.get(position).isExerciseDone() == true) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek3Day2Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
