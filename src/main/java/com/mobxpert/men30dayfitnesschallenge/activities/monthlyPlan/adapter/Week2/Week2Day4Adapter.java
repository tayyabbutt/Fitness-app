...
public class Week2Day4Adapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public Week2Day4Adapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
     ...
     /*   if (myWorkoutList.get(position).isExerciseDone() == true) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveWeek2Day4Workout(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
