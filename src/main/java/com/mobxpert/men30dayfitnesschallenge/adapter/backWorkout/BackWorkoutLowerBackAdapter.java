...
public class BackWorkoutLowerBackAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public BackWorkoutLowerBackAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
        ...
        if (myWorkoutList.get(position).isExerciseDone() == true) {
            holder.parenLayout.setBackgroundColor(Color.parseColor("#567845"));
        }

        if (isExerciseDone == true && itemPosition == position) {
            myWorkoutList.get(position).setExerciseDone(true);
            MyDbPrefrences.saveBackWorkoutLowerBack(context, myWorkoutList);
