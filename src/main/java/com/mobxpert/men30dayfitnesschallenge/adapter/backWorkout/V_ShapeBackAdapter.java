package com.mobxpert.men30dayfitnesschallenge.adapter.backWorkout;
...
public class V_ShapeBackAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public V_ShapeBackAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private class MyViewHolder extends RecyclerView.ViewHolder {
        TextView bicepsName,compeleteText;
        LinearLayout parenLayout;

        public MyViewHolder(View itemView) {
            super(itemView);
            bicepsName = itemView.findViewById(R.id.bicepsName);
            compeleteText = itemView.findViewById(R.id.doneText);
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
        ...
        if (myWorkoutList.get(position).isExerciseDone() == true) {
            holder.compeleteText.setVisibility(View.VISIBLE);
        }

        if (isExerciseDone == true && itemPosition == position) {
            myWorkoutList.get(position).setExerciseDone(true);
            MyDbPrefrences.saveBackWorkoutVShapeBack(context, myWorkoutList);
            holder.compeleteText.setVisibility(View.VISIBLE);
        } else {
            holder.bicepsName.setText(myWorkoutList.get(position).getName());
        }
