package com.mobxpert.men30dayfitnesschallenge.adapter.shoulderWorkout;
...
public class FrontShoulderAdapter  extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public FrontShoulderAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveShoulderWorkoutFrontShoulder(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
        } else {
            holder.bicepsName.setText(myWorkoutList.get(position).getName());
        }
