package com.mobxpert.men30dayfitnesschallenge.adapter.legsWorkout;
...
public class FrontThighsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public FrontThighsAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveLegsWorkoutFrontThighs(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
        } else {
            holder.bicepsName.setText(myWorkoutList.get(position).getName());
        }
