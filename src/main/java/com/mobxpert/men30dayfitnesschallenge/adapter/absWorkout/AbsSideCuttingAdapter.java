package com.mobxpert.men30dayfitnesschallenge.adapter.absWorkout;
...
public class AbsSideCuttingAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AbsSideCuttingAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveAbsWorkoutSideCuttingAbs(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
        } else {
            holder.bicepsName.setText(myWorkoutList.get(position).getName());
        }
