...
public class AbsUperAbsAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    public AbsUperAbsAdapter(Context context, List<MyWorkout> myWorkoutList, int itemPosition, boolean isExerciseDone, OnMyWorkoutClickListner onMyWorkoutClickListner) {
    ...
    private void bindBodyComponent(MyViewHolder holder, final int position) {
        ...
        if (isExerciseDone == true && itemPosition == position) {
            ...
            MyDbPrefrences.saveAbsWorkoutUpperAbs(context, myWorkoutList);
            holder.completeText.setVisibility(View.VISIBLE);
        } else {
            holder.bicepsName.setText(myWorkoutList.get(position).getName());
        }
